...
const resolvers = {
  ...
  Mutation: {
    ...
    addTea: async (_: any, { name, brand, type, imageUrl, tastingNotes, tags }: any, context: any) => {
      if (!context.req.user) {   // ✅ correct: req.user
    ...
    deleteComment: async (_: any, { spillPostId, commentId }: any, context: any) => {
      if (!context.user) {
        throw new AuthenticationError('Authentication required');
      }
      return SpillPost.findByIdAndUpdate(
        spillPostId,
        { $pull: { comments: { _id: commentId, createdByUsername: context.user.username } } },
        { new: true }
      );
    }, 
  },
